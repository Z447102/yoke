...
| 主营业务选择 | `CreateBusinessPage` | 必填，点击后跳转主营业务选择页；类目数据按行业由后端返回 |
...
- 主营业务选择页顶部展示返回按钮、标题“主营业务”和当前行业；主体按层级展示后端返回的类目树。
- 类目数据与所选行业挂钩，进入页面后通过行业请求后端类目树；具体层级数量由后端返回决定，前端不写死级数。
- 用户选择一级类目后才展示二级类目，选择二级后才展示三级，以此类推。
- 当前层级存在子级时，选中后重置后续层级选择，并继续展示下一层；当前层级无子级时视为末级已选。
- 只有选择到末级类目后，“确定”按钮才变为可点击状态；未选完时按钮保持置灰并拦截点击。
- 点击“确定”后返回上一页；后续接入状态共享时通过 Pinia 或页面事件回填完整 `businessType` 路径。
...
- 点击主营业务按钮能进入主营业务选择页，并按当前行业加载对应类目树。
- 主营业务选择页按层级逐级展示类目；选择上一级后才出现下一级。
- 未选择到末级类目时“确定”按钮置灰且不可提交，选择末级后按钮变为橙色并可点击。
- 必填项未填写时点击“下一步”有明确提示。
- 清空主营业务范围按钮可正常清空输入。
