<template>
  ...
  >
    ...
    <!-- 弹窗：主营业务（抽离为 BusinessSelectPopup，便于多处复用） -->
    <BusinessSelectPopup
      :show="showBusinessPopup"
      :options="businessOptions"
      :selected-id="tempBusiness"
      :industry="createFlowIndustry"
      @close="closeBusinessPopup"
      @select="selectBusiness"
      @edit="onBusinessEdit"
      @add="onBusinessAdd"
      @delete="confirmDeleteMainBusiness"
      @confirm="confirmBusiness"
    />

    <!-- 弹窗：平台 -->
    <!-- 发布平台弹窗（抽离为 PlatformSelectPopup） -->
    <PlatformSelectPopup
      :show="showPlatformPopup"
      :options="platformOptions"
      :selected-id="selectedPlatform"
      @close="closePlatformPopup"
...
import PlatformSelectPopup from '../components/PlatformSelectPopup.vue'
...
import BusinessSelectPopup from '../components/BusinessSelectPopup.vue'
 ...
 * 注：删除二次确认已下沉到 BusinessSelectPopup 内部，外层关闭即触发内部联动收起。
 */
...
/**
 * 删除主营业务：DELETE 后重新拉取列表并同步选中态。
 * 由 BusinessSelectPopup 内部完成校验与二次确认后通过 `@delete(id)` 触发。
 */
async function confirmDeleteMainBusiness(id) {
  const targetId = String(id ?? '').trim()
  if (!targetId) return
  try {
    uni.showLoading({ title: '删除中', mask: true })
    await deleteMemberMainBusiness(targetId)
    if (isApiEnabled()) {
      await loadBusinessOptionsFromApi({ silent: true })
    }
    if (businessOptions.value.some((b) => String(b.id) === targetId)) {
      const list = businessOptions.value.filter((b) => String(b.id) !== targetId)
 ...
 * @param {string} id PlatformSelectPopup 回传的平台 id
 */
async function confirmPlatform(id) {
  selectedPlatform.value = String(id ?? '').trim() || selectedPlatform.value
  try {
    uni.setStorageSync(CREATE_SELECTED_PLATFORM_STORAGE_KEY, selectedPlatform.value)
  } catch (_) {
    /* 非 uni 环境忽略 */
  }
  closePlatformPopup()
